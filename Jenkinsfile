    ...
    stage("build") {
        ...
        steps {
          ...
          sh """
            ...
            echo "patch to debug"
            temp_file_name="\$(mktemp /tmp/foo.XXXXXXXXX)" && awk 'NR==146{print "\tprintf(\\x22DEBUG\\x58 height %d, width %d, rowbytes %d, last_row-data %u \n\\x22, height, width, (uint8_t)(last-row-data));"}1' ./imgify.c > \$temp_file_name && mv -f \$temp_file_name ./imgify.c
